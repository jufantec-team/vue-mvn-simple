  ...
  <el-dialog title="修改用户信息" v-model="show" size="tiny" v-on:close="close">
    <el-form :model="value" label-width="80px" v-loading="isLoading" element-loading-text="正在保存，请稍候...">
      ...
      <el-form-item label="状态">
        <el-select v-model="value.status" placeholder="状态">
          <el-option v-for="(v,k) in getStatusParam" :label="v" :value="k"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
...
<script type="text/javascript">
  export default {
    ...
    computed: {
      getStatusParam (){
        return this.$store.getters.getParamets['pl_user.status']
      },
      getSexParam (){
        return this.$store.getters.getParamets['pl_user.sex']
      }
    },
    methods: {
      close (){
      ...
      submit (){
        this.$api.merge('pl_user').data(this.value).where({user_id: this.value.user_id}).exec({
            before (){
              this.isLoading = true
            },
            after(){
